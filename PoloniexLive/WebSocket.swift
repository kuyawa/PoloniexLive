//
// SwiftWebSocket by Tidwall
// https://github.com/tidwall/SwiftWebSocket
//
// Requires Libz.dylib (Libz.tbd) in Project -> Targets -> General -> Linked Frameworks and Libraries
//
...
private class InnerWebSocket: Hashable {
    ...
    func close(_ code : Int = 1000, reason : String = "Normal Closure") {
        let f = Frame()
        f.code = .close
        f.statusCode = UInt16(code)  //truncatingBitPattern: code) //(truncatingIfNeeded: code)
        f.utf8.text = reason
        sendFrame(f)
    }
